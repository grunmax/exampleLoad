...
class LoadScript extends Simulation{
  val users = csv(new java.io.File(".").getCanonicalPath + "/test_users.csv").circular
  val httpConf = http
  ...
  val basicLoad = scenario("BASIC_LOAD").feed(users).during(1 minutes) {
    exec(BasicLoad.start)
  }
  setUp(
    basicLoad.inject(rampUsers(10) over (2 minutes))
      .protocols(httpConf))
